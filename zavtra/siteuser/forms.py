...
class ZRadioRenderer(RadioFieldRenderer):
    def render(self):
	return u'<div class="controls">\n%s\n</div>' % u'\n'.join([u'%s' % force_unicode(w).replace('<label ', '<label class="radio" ') for w in self])
...
class RegistrationForm(forms.Form):
    ...
    about      = forms.CharField(label=u'О себе', widget=forms.Textarea(attrs={'rows': 10, 'class': 'span6'}), required=False)
