...
knownFullExceptions = map T.pack exceptions
    where exceptions = ["ii съезд нпср", "студент", "профессор", "журналист", "д.и.н.", "д.м.н", "д.ф.н.", "д.э.н.", "дед из сибири", "депутат госдумы"] ++
		       ["депутат госдумы рф", "депутат госдумы россии", "депутат госдумы от фракции кпрф", "депутат государственной думы"] ++
		       ["депутат государственной думы рф", "профессор мгу", "доктор исторических наук", "доктор медицинских наук", "доктор права"] ++
		       ["доктор технических наук", "доктор философских наук", "доктор экономических наук", "доцент", "эурналист", "инженер-физик"] ++
		       ["кандидит биологических наук", "кандидат наук", "кандидат философских наук", "капитан 1 ранга", "капитан i ранга"] ++
		       ["адвокат", "актер", "юрист", "режиссер", "редакция", "сербский поэт", "старший эксперт"]

knownInitialExceptions :: [T.Text]
knownInitialExceptions = map T.pack exceptions
    where exceptions = ["специальный корреспондент", "сопредседатель координационного совета", "собственный кореспондент"]
